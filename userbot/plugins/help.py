...
async def cmd_list(event):
    ...
    if input_str == "text":
        string = (
            "Total {count} commands found in {plugincount} plugins of RDX SPAM BOT\n\n"
        )
        catcount = 0
        plugincount = 0
        ...
        if len(string) > 4095:
            ...
            reply_text = f"**All commands of the  RDX SPAM BOT can be seen [here]({url})**"
...
async def info(event):
    ...
    input_str = event.pattern_match.group(1)
    if input_str == "text":
        string = "Total {count} commands found in {plugincount} sudo plugins of  RDX SPAM BOT\n\n"
        catcount = 0
        plugincount = 0
